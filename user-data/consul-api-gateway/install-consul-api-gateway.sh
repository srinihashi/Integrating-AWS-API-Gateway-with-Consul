#!/bin/bash
set -e
SRV="consul-api-gateway"
SRC_DIR="/tmp/${SRV}"
##CERTS_DIR="/tmp/consul-common/certs"
sleep 30s
echo "Installing Consul API Gateway..." > ${SRC_DIR}/install-progress.log

# Install unzip
echo "Installing unzip pkg..." > ${SRC_DIR}/install-progress.log
...
consul connect envoy -gateway api -register -service ${SRV} -envoy-binary /usr/local/sbin/envoy >> ${SRC_DIR}/envoy-proxy.log 2>&1 &
...
# Consul API Gateway
#####################################################################
echo "Configuring Consul API Gateway..." >> ${SRC_DIR}/install-progress.log
# Apply proxy defaults config
consul config write ${SRC_DIR}/proxy-defaults.hcl
echo "Configured Consul Proxy defaults..." >> ${SRC_DIR}/install-progress.log

# Config Consul API Gateway
consul config write ${SRC_DIR}/api-gateway-config.hcl
echo "Configured Consul API Gateway..." >> ${SRC_DIR}/install-progress.log

# Apply Route
consul config write ${SRC_DIR}/routes.hcl
echo "Configured Consul http-route..." >> ${SRC_DIR}/install-progress.log

# Apply Intention Config
consul config write ${SRC_DIR}/intentions.hcl
echo "Configured service intentions..." >> ${SRC_DIR}/install-progress.log

echo "Done configuring Consul API Gateway..." >> ${SRC_DIR}/install-progress.log